...
from app.models.enums import DeviceClaimStatus, DeviceStatus, enum_values
...
class Device(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint(f"status IN ({enum_values(DeviceStatus)})", name="status"),
        CheckConstraint(
        ...
        Index("ix_devices_owner_user_id", "owner_user_id"),
        Index(
    ...
    device_token_hash: Mapped[str | None] = mapped_column(String(64))
    status: Mapped[str] = mapped_column(
        String(16), nullable=False, server_default=DeviceStatus.UNCLAIMED.value
    )
    firmware_version: Mapped[str | None] = mapped_column(String(32))
    claimed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    # 서버(백엔드)가 갱신한다. 수집 Lambda(sensor_ingest)는 devices를 수정하지 않는다.
    last_seen_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))


class DeviceClaim(Base, UUIDPrimaryKeyMixin):
    """기기 claim "시도". `devices.owner_user_id`(현재 실제 소유자)와 의미가 다르다."""

    __tablename__ = "device_claims"
    __table_args__ = (
        CheckConstraint(f"status IN ({enum_values(DeviceClaimStatus)})", name="status"),
        CheckConstraint(
        ...
        Index("ix_device_claims_user_id", "user_id"),
    ...
    device_id: Mapped[str] = mapped_column(
        String(12), ForeignKey("devices.id", ondelete="CASCADE"), nullable=False
    )
    ...
    status: Mapped[str] = mapped_column(
        String(16), nullable=False, server_default=DeviceClaimStatus.PENDING.value
    )
...
class PlantDevice(Base):
    ...
    device_id: Mapped[str] = mapped_column(
        String(12), ForeignKey("devices.id", ondelete="CASCADE"), primary_key=True
    )
