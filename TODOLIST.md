...
### 센서 기기 등록 (2026-09-17 마이페이지 시안 대조에서 추가)

| 항목 | 자리 | 내용 |
|---|---|---|
| 센서 기기 등록 메뉴 | `my_page_screen.dart` `ProfileMenuCard`, `_registerSensor` | 시안(4534:3044) 메뉴 3번째 '센서 기기 등록' 행 **추가 완료**. 탭하면 '곧 제공돼요' 토스트로 막아 둠. 등록 플로우가 나오면 이 토스트를 화면 전환으로 바꾼다 |
| 센서 기기 등록 플로우 12화면 | (신규 화면들) | 디자이너가 **시안 완성(2026-09-17, 노드 `4916:5` "기기연결")**. BLE 스캔→기기확인(LED)→Wi-Fi 연결→설정 진행→등록 완료→연결할 식물 선택, 실패 분기(기기 못찾음/Wi-Fi 실패/서버등록 실패) 포함. **미룸**: BLE·Wi-Fi 프로비저닝·서버등록이 전부 센서 담당 #52(@jdk829355)의 펌웨어·백엔드에 묶여 있어, UI만 먼저 만들면 입력 구조가 계약 따라 바뀌어 재작업 위험이 크다. #52 계약 확정 후 착수. 착수 시 뼈대 4화면(찾기·확인·진행·완료)부터 |
