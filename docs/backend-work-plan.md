...
- `client_registration_id`로 모바일 재전송과 동시 등록 중복 방지
...
- 물주기와 분갈이 자동 반복
- enabled 반복 규칙마다 다음 `SCHEDULED` 이벤트 하나 유지
- 분갈이 `KNOWN`은 입력 날짜, `NEVER`는 `started_on`을 최초 일정 기준일로 사용
- 과거인 최초 분갈이 예정일은 주기 단위로 오늘 이후까지 이동
- `performed_on`과 서버 `recorded_at` 분리
- 과거 수행일 허용, 미래 수행일 차단
- 실제 수행일을 기준으로 다음 일정 생성
- 중복 완료 요청 멱등 처리
- 비료·가지치기·자유 할 일은 오늘·미래 일회성 일정만 허용
- 사용자 일회성 일정은 `client_event_id`와 요청 해시로 생성 재전송 멱등 처리
- 홈 메모는 오늘만 작성·수정·삭제하며 본문 `1~500자`
...
- 물주기·분갈이 예정·지연 알림은 사용자 현지 오전 9시에 일정별 한 번 발송
- 진단 완료 알림
- 모든 식물 알림함과 읽음 처리
- 사용자 전체 푸시 ON/OFF
- 활성 기기 FID unique
