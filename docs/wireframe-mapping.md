...
| AI 대화 | 선택 식물의 단일 채팅방 자동 표시, 사진 한 장 첨부, Tool Calling으로 식물·관리 기록 조회 | `/plants/{id}/chat`, `/plants/{id}/chat/messages` | `AI_CHATS`, `AI_MESSAGES`, `AI_TOOL_CALLS` | 태그·새 채팅 UI 제거 필요 |
...
| 대화 목록 | 사용하지 않음. 식물 전환이 곧 채팅방 전환이며 식물별 채팅방은 하나만 유지 | 없음 | `AI_CHATS.plant_id` unique | 화면 제거 필요 |
...
4. 식물마다 채팅방은 정확히 하나이며 식물 등록 시 자동 생성합니다.
5. 다른 식물을 선택하면 그 식물의 기존 채팅방으로 전환하며 태그 선택과 새 채팅
   기능은 제공하지 않습니다.
...
7. 대화 목록 화면과 `설정`·검색·새 채팅 버튼을 제거하고 현재 선택 식물의
   채팅방을 바로 표시합니다.
