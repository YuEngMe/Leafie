...
- 지연 일정의 `due_date`를 매일 미루지 않으며 완료할 때까지 `OVERDUE`로 계산합니다.
- 캘린더의 월·주 화면은 같은 날짜 범위 API를 사용합니다. 물주기, 분갈이, 비료,
  가지치기와 다이어리 컨디션을 평탄화한 배열로 반환하며, 미확정 `CUSTOM` 일정은
  캘린더에서 제외합니다.
- 소유 식물이 없으면 홈은 `200 OK`와 null 식물 컨텍스트를 반환합니다.
- 선택 식물 삭제 시 가장 오래된 남은 식물을 선택하고, 없으면 선택을 null로 만듭니다.
- 홈 대사는 성격, 오늘 컨디션과 일정 상태에 맞는 고정 문구 중 하나를 선택합니다.
  문구 목록과 컨디션이 없을 때의 기본 표정이 확정되기 전에는 해당 값을 null로 반환합니다.
...
모바일 재전송은 대화별 `client_message_id`로 중복 호출을 차단합니다. 서버는 사용자별
최근 1분과 24시간 채팅 횟수, 24시간 진단·사진 인식 횟수를 DB에서 확인해 비용 상한을
적용합니다.
...
1. 로컬에서 파일 형식, 크기, 흐림과 밝기 검사
2. `DiagnosisProvider`에서 식물 존재와 건강 상태 판정
3. Provider 응답을 내부 schema로 표준화
4. 종별 진단 규칙, 환경과 최근 관리 이력으로 추천 관리 보강
5. 채팅 결과 카드, 진단 이력과 푸시 알림 생성

진단 결과 설명을 위한 별도 LLM 호출은 하지 않습니다. 상태와 원인 확률은 전문
Provider 결과만 사용하고, 추천 관리는 검토된 내부 규칙으로 보강합니다. 촬영 화면에서
증상 부위를 가까이 담도록 안내하되 서버가 증상 부위 노출 여부를 임의로 확정하지 않습니다.
