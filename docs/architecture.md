...
→ 장소 별명, 식물 시작일, 마지막 물 준 날, 선택적 분갈이 날짜 입력
→ 성격 선택
→ 컬러·헤어 선택
→ 최종 확인 및 등록
```
...
- 상단 `D+`는 식물 시작일로 계산합니다.
- 배경은 사용자 시간대의 시간 구간에 따라 낮·밤 등으로 바뀝니다.
...
습도·조도에 따른 표정과 센서 알림은 센서 도메인이 산출한 결과를 표시만 합니다.
홈 API는 센서 원시값이나 판정 공식을 소유하지 않습니다.
...
- Worker는 예정 시각 이후 편지를 가져와 식물 정보, 성격, 다이어리와 센서 담당자가
  제공한 해당 날짜 요약을 바탕으로 편지를 생성합니다.
- 편지는 다이어리당 정확히 한 통입니다.
- 생성 전 다이어리를 수정하면 Worker가 최신 내용을 읽습니다.
- 편지 생성 후 다이어리를 수정해도 기존 편지는 변경하거나 재생성하지 않습니다.
...
장치 등록, Wi-Fi 연결, 측정 주기, 원시 습도·조도 저장, 하루 권장 조도 누적 계산,
